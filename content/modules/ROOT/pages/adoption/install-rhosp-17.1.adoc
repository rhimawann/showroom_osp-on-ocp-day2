...
. Access the bastion by executing the following command:
+
...
----
+
The SSH password is `{bastion_ssh_password}`. The UUID of your lab is: `{guid}`.

If needed, you can navigate to the OpenShift console URL: {openshift_cluster_console_url}[{openshift_cluster_console_url}^] using the user `admin` and password `{KpdAM0JNSY9hvuH4}`.
