...
* Create the dataplane network (IPAM):
+
----
...
--save-config \
--dry-run=client \
--from-file=authorized_keys=/home/lab-user/.ssh/t5ppwkey.pub \
--from-file=ssh-privatekey=/home/lab-user/.ssh/t5ppwkey.pem \
--from-file=ssh-publickey=/home/lab-user/.ssh/t5ppwkey.pub \
-n openstack \
-o yaml | oc apply -f-
----
TODO
...
* Take some time to go through the *osp-ng-dataplane-adoption-compute-2.yaml* file. We have configured the edpm_ovn_bridge_mappings with "datacentre:br-ex"
...
kind: OpenStackDataPlaneDeployment
metadata:
  name: openstack-edpm-compute-2-adoption
spec:
  nodeSets:
  - openstack-edpm-compute-2
EOF
